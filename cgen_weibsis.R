...
gfunc_const <- function(dose0,dose1,rlambda1,rlambda2,rbeta){
  ...
  t1 <- log(1-runif(1))/(-rlambda1*dose0); gendat$t1 <- t1
  t2 <- log(1-runif(1))/(-rlambda2*dose0); gendat$t2 <- t2
  t3 <- log(1-runif(1))/(-rlambda2*dose1^exp(rbeta)); gendat$t3 <- t3
...
stancode_weibull <- readLines("stancode_weibull_uninform.txt")
mymodel <- stan_model(model_code=stancode_weibull)
...
changegrid <- data.frame(matrix(nrow=54,ncol=8))
names(changegrid) <- c("generator","type","sampsize","lambda1","lambda2","beta","DRtarget","DLTtarget")
changegrid$sampsize <- c(rep(101,18),rep(61,18),rep(31,18))
changegrid$generator <- rep("constant",nrow(changegrid))
changegrid$type <- rep(c(rep("null",6),rep("DR35",6),rep("DR65",6)),3)
changegrid$lambda1 <- rep(c(0.162,0.042,0.017,0.111,0.029,0.0113,0.1,0.0255,0.0105,0.062,0.0156,0.0063,0.248,
                            0.066,0.027,0.174,0.0447,0.0181),3)
changegrid$lambda2 <- rep(c(0.058,0.0175,0.0074,0.038,0.011,0.0045,0.061,0.0175,0.0073,0.04,0.0108,0.0045,
                            0.0545,0.0177,0.0077,0.036,0.011,0.0047),3)
changegrid$beta <- -0.5
...
runSim <- function(n, N){
  ...
  for(nsim in 1:N){
    ...
    frame <- data.frame(matrix(nrow=sampsize, ncol=42+length(doses)*2))
    a_percs <- paste("a_",c("2.5","25","75","97.5"),sep='')
    ...
    names(frame) <- c("Patient","y1","delta1","y2","delta2","dose0","dose1","t1","t2","t3",
                      ...
                      "current_y1","current_y2","a","l1","l2","beta",a_percs,l1_percs,l2_percs,beta_percs, 
    ...
    for(i in 1:sampsize){
      ...
      temp <- gfunc_const(dose0,dose1,rlambda1,rlambda2,rbeta)
      
      
      #record accrual time and total time
      frame$Patient[i] <- i
      newtime <- rgeom(1,1/(accrualmean+1))/7
      frame$accrualadd[i] <- ifelse(newtime==0,1/7,newtime)
      ...
      if(i>1){
        ...
        #store model results
        a <- median(result$alpha)
        l1 <- median(result$lambda1)
        l2 <- median(result$lambda2)
        b <- median(result$beta)
        frame[i,19:22]<- c(a,l1,l2,b)
        frame[i,23:26] <-  quantile(result$alpha, c(0.025,0.25,0.75,0.975))
        frame[i,27:30] <-  quantile(result$lambda1, c(0.025,0.25,0.75,0.975))
        frame[i,31:34] <-  quantile(result$lambda2, c(0.025,0.25,0.75,0.975)) 
        frame[i,35:38] <- quantile(result$beta, c(0.025,0.25,0.75,0.975)) 
        #calculate and store probabilities
        drprobs <- h_dr_weib(targettime,a,l1,l2,doses[1:(length(doses)-1)])
        dltprobs <- h_dltany_weib(targettime,a,l1,l2,b,doses[1:(length(doses)-1)],doses[2:length(doses)])
        frame[i,39:(39+length(doses)-2)]<-drprobs
        frame[i,(39+length(doses)-1):(42+length(doses))]<-dltprobs
        #see if there are any monotonicity inconsistencies in calculated probabilites, DLT higher for smaller dose.
        if((frame[i,44]<frame[i,45])||
           (frame[i,45]<frame[i,46])||
           (frame[i,46]<frame[i,47])||
           (frame[i,47]<frame[i,48])){
        ...
        dose0 <- doses[which((abs(drprobs-DRtarget)+3*abs(dltprobs-DLTtarget))==min((abs(drprobs-DRtarget)+3*abs(dltprobs-DLTtarget))))]
...
library(parallel)
resultSet9 <- mclapply(c(1:54), runSim, N=100, mc.cores = numCores)
save(resultSet9,file="/home/jeh6kw/results_cgen_weibsis_9.Rdata")
Sys.time()-r1