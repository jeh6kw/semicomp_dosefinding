...
changegrid <- data.frame(matrix(nrow=36,ncol=10))
names(changegrid) <- c("generator","type","sampsize","alpha","lambda1","lambda2","beta0","beta1",
                       "DRtarget","DLTtarget")
changegrid$sampsize <- c(rep(101,12),rep(61,12),rep(31,12))
changegrid$generator <- "weibull"
changegrid$type <- rep(c(rep("inc_small",6),rep("dec_big",6)),3)
changegrid$alpha <- rep(c(rep(1.1,6),rep(0.5,6)),3)
changegrid$lambda1 <- rep(c(0.012,0.0083,0.0038,0.0074,0.0053,0.0025,0.08,0.058,0.026,0.06,0.044,0.0195),3)
changegrid$lambda2 <- rep(c(0.005,0.0033,0.0016,0.003,0.002,0.001,0.043,0.03,0.014,0.028,0.02,0.009),3)
changegrid$beta0 <- -0.5
changegrid$beta1 <- 2
changegrid$DRtarget <- rep(c(rep(50,3),rep(40,3)),6)
changegrid$DLTtarget <- rep(c(rep(30,3),rep(20,3)),6)
...
library(parallel)
resultSet <- mclapply(c(1:36), runSim, N=3, mc.cores = numCores)
save(resultSet,file="/scratch/jeh6kw/results_weibbdincgen_constanalysis_uninform.Rdata")
Sys.time()-r1