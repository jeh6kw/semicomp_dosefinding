...
changegrid <- data.frame(matrix(nrow=18,ncol=8))
names(changegrid) <- c("generator","type","sampsize","lambda1","lambda2","beta","DRtarget","DLTtarget")
changegrid$sampsize <- c(rep(101,6),rep(61,6),rep(31,6))
changegrid$generator <- "constant"
changegrid$type <- "null"
changegrid$lambda1 <- rep(c(0.162,0.042,0.017,0.111,0.029,0.0113),3)
changegrid$lambda2 <- rep(c(0.058,0.0175,0.0074,0.038,0.011,0.0045),3)
changegrid$beta <- -0.5
changegrid$DRtarget <- rep(c(rep(50,3),rep(40,3)),3)
changegrid$DLTtarget <- rep(c(rep(30,3),rep(20,3)),3)
...
library(parallel)
resultSet <- mclapply(c(1:18), runSim, N=3, mc.cores = numCores)
save(resultSet,file="/scratch/jeh6kw/results_titecrm_constantgen.Rdata")
Sys.time()-r1
